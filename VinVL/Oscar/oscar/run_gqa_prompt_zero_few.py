...
from oscar.modeling.modeling_bert import ImageBertForSequenceClassification, \
    BertImgForPreTraining, BertImgModel, BertPreTrainingHeads
...
class ImageBertForSequenceClassificationPromptSubset(BertImgForPreTraining):
    ...
    def init_weights_with_pretrained_parameters(self, config):
        ...
        assert len(base_vocab) == base_weight.shape[0] and \
            config.num_labels == self.cls_ans.predictions.decoder.weight.data.shape[0] and \
               len(base_vocab) == base_bias.shape[0]
...
MODEL_CLASSES = {
    'bert': (BertConfig, ImageBertForSequenceClassificationPromptSubset, BertTokenizer),
}
